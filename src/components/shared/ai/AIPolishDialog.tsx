...
const createMockPolishedContent = (source: string, customInstructions: string) => {
  const trimmed = source.trim();
  const base = trimmed || "- 请在这里填写需要润色的简历内容。";
  return [
    base,
    "",
    "> 开发模拟模式：以上内容已按“表达更专业、重点更突出、结构更清晰”的方向生成示例润色稿。",
    customInstructions.trim() ? `> 已参考额外要求：${customInstructions.trim()}` : "",
  ].filter(Boolean).join("\n");
};
...
export default function AIPolishDialog({
  ...
  const handlePolish = async () => {
    try {
      ...
      if (mockAIEnabled) {
        const mockContent = createMockPolishedContent(
          turndownService.turndown(content),
          customInstructions
        );
        await streamMockText(mockContent, (chunk) => {
