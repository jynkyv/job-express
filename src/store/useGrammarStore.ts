...
export const useGrammarStore = create<GrammarStore>((set, get) => ({
  ...
  checkGrammar: async (text: string) => {
    ...
    try {
      if (mockAIEnabled) {
        const mockErrors: GrammarError[] = text.includes("做为")
          ? [{
              context: "检测到“做为”这一常见错别字。",
              text: "做为",
              suggestion: "作为",
              reason: "错别字",
              type: "spelling",
        ...
        if (preview) {
          ...
          marker.unmark();
          mockErrors.forEach((error) => markSingleError(marker, error));
        }
        toast.success(mockErrors.length ? "开发模拟模式：已返回示例校对结果" : "开发模拟模式：未发现语法错误");
        return;
      }
