...
import MockInterview from "@/features/career/components/MockInterview"
import { ArrowLeft, Settings } from "lucide-react"

export default function InterviewPage() {
  const { navigateTo, openSettings } = useAppContext()
    ...
    <div className="relative flex h-screen flex-col bg-[#eef4fb]">
      <header className="pointer-events-none fixed left-0 right-0 top-0 z-30 flex h-16 items-center justify-between bg-transparent px-6 text-slate-800">
        ...
        <div className="text-center">
          <p className="text-sm font-semibold">模拟面试</p>
          <p className="text-xs text-slate-500">Interview Studio</p>
        </div>
        <button
      ...
      <MockInterview onOpenSettings={openSettings} />
    </div>
  )
}
