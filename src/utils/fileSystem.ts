...
export const storeFileHandle = async (
  ...
  const activeDb = db;
  return new Promise((resolve, reject) => {
    const transaction = activeDb.transaction(HANDLE_STORE, "readwrite");
...
export const getFileHandle = async (
  ...
  const activeDb = db;
  return new Promise((resolve, reject) => {
    const transaction = activeDb.transaction(HANDLE_STORE, "readonly");
    const store = transaction.objectStore(HANDLE_STORE);
...
export const storeConfig = async (key: string, value: any): Promise<void> => {
  ...
  const activeDb = db;
  return new Promise((resolve, reject) => {
    const transaction = activeDb.transaction(CONFIG_STORE, "readwrite");
...
export const getConfig = async (key: string): Promise<any> => {
  ...
  const activeDb = db;
  return new Promise((resolve, reject) => {
    const transaction = activeDb.transaction(CONFIG_STORE, "readonly");
    const store = transaction.objectStore(CONFIG_STORE);
    const request = store.get(key);
